...
This final verification demonstrates the power of vCluster's isolation:
- The same application name (`my-app`) works in both vClusters
- Each instance is completely independent and isolated
- The applications can be accessed simultaneously from the same host
- Each vCluster maintains its own control plane, API, and networking
- This shows that vCluster truly provides multi-tenancy at the control plane level

This capability is crucial for organizations that want to share infrastructure while maintaining tenant independence and avoiding naming conflicts.